...
IOU_THRESHOLD = 0.5
NMS_THRESHOLD = 0.4
MIN_DISTANCE = 50  # Minimum distance between trackers to avoid duplicates
...
def process_video(video_path, cascade_path, status_placeholder, progress_bar, debug_placeholder):
    ...
    if not video.isOpened():
        st.error("Error opening video file. Check if the file is valid or supported.")
        return
    ...
    while 'running' in st.session_state and st.session_state.running:
        ...
        rc, image = video.read()
        if not rc:
            status_placeholder.write("Video ended or error reading frame.")
        ...
        if not (frameCounter % 10):
            ...
            if len(cars) > 0:
                ...
                scores = [(w * h) / (WIDTH * HEIGHT) for _, _, w, h in cars]  # Approximate confidence
                ...
                # Handle indices as tuple or list
                if isinstance(indices, tuple):
                    indices = indices[0]  # Extract indices if tuple
                indices = np.array(indices).flatten() if isinstance(indices, np.ndarray) else list(indices)
                cars = [boxes[i] for i in indices]
...
def main():
    ...
    st.write("This app processes a video to track vehicles and estimate their speeds using Haar Cascade and OpenCV CSRT trackers.")
    ...
    # Status, progress, and debug placeholders
    ...
    if st.button("Start/Stop Video Processing", disabled=not video_file):
        ...
        if not st.session_state.running:
            status_placeholder.write("Video processing stopped.")
            progress_bar.progress(0.0)
    ...
    if st.session_state.running and video_file:
        ...
        status_placeholder.write("Upload a video and press the button to start processing.")
